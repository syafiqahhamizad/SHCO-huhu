...
export const StaffPortalView: React.FC = () => {
  ...
  const canEditProfile = Boolean(isAdmin || currentUser.isSuperAdmin);
      ...
      {activeTab === 'profile' && <form onSubmit={saveProfile} className="bg-white border border-[#E1DCCF] rounded-xl p-5 grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="md:col-span-2 rounded-xl border border-amber-200 bg-amber-50/50 p-4 space-y-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <label className="font-bold text-slate-700">Call-to-the-bar date<input readOnly={!canEditProfile} type="date" value={profile.callToBarDate || ''} onChange={(e) => setProfile({ ...profile, callToBarDate: e.target.value })} className="w-full mt-1" /></label>
          </div>
          <label className="flex items-start gap-2 text-slate-700 cursor-pointer"><input disabled={!canEditProfile} type="checkbox" checked={Boolean(profile.celebrationOptOut)} onChange={(e) => setProfile({ ...profile, celebrationOptOut: e.target.checked })} className="mt-0.5" /><span><strong>Hide my birthday and milestone announcements</strong><span className="block text-[11px] text-slate-500 mt-0.5">The staff record remains stored, but celebrations will not appear on the internal Start Centre.</span></span></label>
        </div>
        <label className="md:col-span-2 font-bold text-slate-700">Professional bio<textarea readOnly={!canEditProfile} rows={4} value={profile.bio} onChange={(e) => setProfile({ ...profile, bio: e.target.value })} className="w-full mt-1" placeholder="Short internal biography, practice focus, and relevant experience" /></label>
